/**
 * Agent 27 — Entry 1 Inscription Script
 *
 * 1. begin-or-get with content hash
 * 2. add-chunk-batch with HTML data
 * 3. seal-recursive with dependency [107] (genesis parent)
 */
...
async function main() {
  ...
  // Step 3: seal-recursive (with genesis dependency [107])
  ...
  const sealTx = await makeContractCall({
    ...
    functionArgs: [
      ...
      listCV([uintCV(GENESIS_TOKEN_ID)])
    ],
    senderKey,
    network,
    nonce,
  ...
  console.log(`Dependencies: [${GENESIS_TOKEN_ID}]`);
}
